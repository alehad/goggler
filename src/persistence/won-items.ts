import { buildHomeFeed } from "../ebay/home-feed.ts";
...
export async function persistWonItemsAndMerge(
  ...
  return rebuildWithPersistedWonItems(
    history,
    persistedWonItems.map((item) => ({
...
function rebuildWithPersistedWonItems(
  history: EbayHistoryResponse,
  wonItems: EbayBuyingHistoryItem[],
  matchingPreferences: MatchingPreferences
): EbayHistoryResponse {
  const homeFeed = buildHomeFeed({
    lostItems: history.lostItems,
    wonItems,
    watchlistItems: history.watchlistItems,
    relistingCandidates: history.relistingCandidates
  });
  ...
  return {
    ...
    counts: {
      ...history.counts,
      won: wonItems.length,
      eventuallyWon: history.lostItems.filter((item) => item.relistingGroupId && wonGroups.has(item.relistingGroupId)).length,
      neverWon: history.lostItems.filter((item) => !item.relistingGroupId || !wonGroups.has(item.relistingGroupId)).length,
